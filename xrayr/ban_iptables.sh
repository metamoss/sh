...
#       Description: iptables 封禁 BT、PT、非法网站、SPAM（垃圾邮件）和自定义端口、关键词
#       Version: 1.0.10
...
Ban_BT(){
	check_BT
	[[ ! -z ${BT_KEY_WORDS} ]] && echo -e "${Error} 检测到已封禁BT、PT、非法网站 关键词，无需再次封禁 !" && exit 0
	s="A"
	Set_BT
	View_ALL
	echo -e "${Info} 已封禁BT、PT、非法网站 关键词 !"
}
Ban_SPAM(){
	check_SPAM
...
Ban_ALL(){
	...
	if [[ -z ${BT_KEY_WORDS} ]]; then
		if [[ -z ${SPAM_PORT} ]]; then
			Set_ALL
			View_ALL
			echo -e "${Info} 已封禁BT、PT、非法网站 关键词 和 SPAM(垃圾邮件) 端口 !"
		else
			Set_BT
			View_ALL
			echo -e "${Info} 已封禁BT、PT、非法网站 关键词 !"
		...
		if [[ -z ${SPAM_PORT} ]]; then
			...
			echo -e "${Error} 检测到已封禁BT、PT、非法网站 关键词 和 SPAM(垃圾邮件) 端口，无需再次封禁 !" && exit 0
		fi
	fi
}
UnBan_BT(){
	check_BT
	[[ -z ${BT_KEY_WORDS} ]] && echo -e "${Error} 检测到未封禁BT、PT、非法网站 关键词，请检查 !" && exit 0
	s="D"
	Set_BT
	View_ALL
	echo -e "${Info} 已解封BT、PT、非法网站 关键词 !"
}
UnBan_SPAM(){
	check_SPAM
...
UnBan_ALL(){
	...
	if [[ ! -z ${BT_KEY_WORDS} ]]; then
		if [[ ! -z ${SPAM_PORT} ]]; then
			Set_ALL
			View_ALL
			echo -e "${Info} 已解封BT、PT、非法网站 关键词 和 SPAM(垃圾邮件) 端口 !"
		else
			Set_BT
			View_ALL
			echo -e "${Info} 已解封BT、PT、非法网站 关键词 !"
		...
		if [[ ! -z ${SPAM_PORT} ]]; then
			...
			echo -e "${Error} 检测到未封禁BT、PT、非法网站 关键词和 SPAM(垃圾邮件) 端口，请检查 !" && exit 0
		fi
	fi
}
...
————————————
  ${Green_font_prefix}1.${Font_color_suffix} 封禁 BT、PT、非法网站
  ${Green_font_prefix}2.${Font_color_suffix} 封禁 SPAM(垃圾邮件)
  ${Green_font_prefix}3.${Font_color_suffix} 封禁 BT、PT、非法网站+SPAM
  ${Green_font_prefix}4.${Font_color_suffix} 封禁 自定义  端口
  ${Green_font_prefix}5.${Font_color_suffix} 封禁 自定义关键词
————————————
  ${Green_font_prefix}6.${Font_color_suffix} 解封 BT、PT、非法网站
  ${Green_font_prefix}7.${Font_color_suffix} 解封 SPAM(垃圾邮件)
  ${Green_font_prefix}8.${Font_color_suffix} 解封 BT、PT、非法网站+SPAM
